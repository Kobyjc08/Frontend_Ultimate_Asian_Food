...
const RenderProduct = ({data}) => {
  ...
  const handlerAddShoppingCart = async() => {
    ...
    let {data:order} =  await axios.get(`https://ultimate-asian-food-e-shop.herokuapp.com/order/${user_id}`);
    
    //validation if data (there are orders for that id? is it empty)
    let newItem;
    if(order.length < 1) { 
      let newOrder = await axios.post('https://ultimate-asian-food-e-shop.herokuapp.com/order', {
        customer_id : user_id
      })
      
      newItem = await axios.post('https://ultimate-asian-food-e-shop.herokuapp.com/orderItems', {
